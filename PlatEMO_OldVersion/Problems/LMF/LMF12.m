...
%--------------------------------------------------------------------------
    ...
    methods
       %% Initialization
        function obj = LMF12()
            % Parameter setting
			obj.type_fm = obj.Global.ParameterSet(2);
            obj.type_lk = obj.Global.ParameterSet(2);
            obj.type_dg = obj.Global.ParameterSet(1);
            obj.type_cv = obj.Global.ParameterSet(1);
            if isempty(obj.Global.M) && obj.Global.M < 3
                obj.Global.M = 3;
            end
            if isempty(obj.Global.D)
                obj.Global.D = 100*obj.Global.M;
            end
            obj.nip = 3*ones(1,obj.Global.M-1);
            obj.nop = 1*ones(1,obj.Global.M-1);
            obj.nsp = 1;
            obj.K = sum(obj.nip) + obj.nsp;
            obj.L = obj.Global.D - obj.K;
            obj.Global.lower = [-1*ones(1,obj.K),zeros(1,obj.L)];
            obj.Global.upper = [ones(1,obj.K),10*ones(1,obj.L)];
            obj.Global.encoding = 'real';
            % Calculate the number of independent distance-related
            % variables in each group
            c1 = 3.8*0.1*(1-0.1);
            c = LogisticMap(c1,3.8,obj.Global.M);
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            if obj.type_lk == 0 % linear linkages
                for i = obj.K+1 : D
                    PopDec(:,i) = (1+(i-obj.K)./obj.L).*(PopDec(:,i)-obj.Global.lower(i)) - yp(:,1).*(obj.Global.upper(i)-obj.Global.lower(i));   
                end 
            elseif obj.type_lk == 1 % nonlinear linkages
                for i = obj.K+1 : D
                    PopDec(:,i) = (1+cos((i-obj.K)./obj.L*pi/2)).*(PopDec(:,i)-obj.Global.lower(i)) - yp(:,1).*(obj.Global.upper(i)-obj.Global.lower(i));
                end
            else                   %mixed linkages
                for i = obj.K+1 : 2 : D
                    PopDec(:,i) = (1+(i-obj.K)./obj.L).*(PopDec(:,i)-obj.Global.lower(i)) - yp(:,1).*(obj.Global.upper(i)-obj.Global.lower(i));
            ...
            a3 = 5;%Define the value of the first entry in the sequence
            d3 = 1;%Define the value of the variance of the sequence
			nk3 = 5; %Number of groups for uniformly deep grouping
            for i = 3 : 3 : M
                if obj.type_dg == 1 %nonuniformly deep grouping without knowing the number of groups
                    dg = DeepGrouping(gd{i},a3,d3);
                else %evenly deep grouping, and the number of groups is preknown, i.e., nk2
                    dg = UniformGrouping(gd{i},nk3);
                end
                yd{i} = zeros(N,length(dg));
                for j = 1 : 2 : length(dg)
...
function f = LogisticMap(c1,r,num)
    f = zeros(1,num);
    f(1) = c1;
    for i = 1 : num-1
        f(i+1) = r.*f(i).*(1-f(i));
...
function gx = Rosenbrock(x,g)
    l = length(g(:));
    [s1,~] = size(x);
    gx = zeros(s1,1);
    for i = 1 : l-1
       gx = gx + 100.*((x(:,g(i)).^2 - x(:,g(i+1))).^2) + (x(:,g(i))-1).^2; 
    end
    gx = gx./l;
end
